class Api::V1::ApiController < ApplicationController
  ...
  def verify_security_hashing!
    ...
    Rails.logger.error("Request contains an invalid security hash. company_id=#{current_company.id} url=#{request.original_url} body=#{valid_url_hash.body_without_hash} provided_hash=#{valid_url_hash.provided_hash} expected_hash=#{valid_url_hash.generated_hash}")
